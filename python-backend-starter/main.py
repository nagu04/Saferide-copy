...
async def generate_report(
    ...
    content = f"Report Type: {type}\nDate Range: {start} to {end}\nGenerated by: {current_user.full_name}"
    file_bytes = content.encode("utf-8")
    file_stream = BytesIO(file_bytes)

    filename = f"Report_{start}_{end}.{format.lower()}"

    MOCK_REPORTS.insert(0, {
        "id": f"REP-{datetime.now().strftime('%Y%m%d%H%M%S%f')}",
        "name": filename,
        "size": f"{len(file_bytes)/1024:.2f} KB",
    ...
    await add_audit_log(
        ...
        user=current_user.full_name,
        details=f"{type} report generated from {start} to {end}"
    )
